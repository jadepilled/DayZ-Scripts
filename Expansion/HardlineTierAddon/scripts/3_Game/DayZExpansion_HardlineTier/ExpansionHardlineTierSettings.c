...
class ExpansionHardlineTierSettings
{
    ...
    protected bool NormalizeColors(out map<string, int> colors, map<string, int> defaults, out ref array<int> cache, int fallback)
    {
        ...
        map<string, int> normalizedColors = new map<string, int>();
        ...
        foreach (string tierKey, int color: colors)
        {
            ...
            if (normalizedColors.Contains(normalizedKey))
            {
                // Last one wins if duplicates exist; mark as updated so we rewrite the map safely later.
                updated = true;
            }
            normalizedColors.Set(normalizedKey, color);
        ...
        for (int tierIdx = 0; tierIdx <= ExpansionHardlineTierAddon.EXPANSION_MAX_TIER; tierIdx++)
        {
            ...
            if (!normalizedColors.Contains(tierString))
            {
                normalizedColors.Insert(tierString, cache[tierIdx]);
                updated = true;
            }
        }

        colors.Clear();
        colors.Copy(normalizedColors);
